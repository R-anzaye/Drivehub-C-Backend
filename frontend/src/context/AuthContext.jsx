...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (first_name, last_name, email, password, organization) => {
    try {
      const response = await fetch('http://127.0.0.1:8080/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ first_name, last_name, email, password, organization }),
      });
      const data = await response.json();

      if (response.ok) {
        toast.success('Registration successful! Logging you in...');
        login(email, password); // Log in after successful registration
      } else {
